        ...
        VStack(spacing: 16) {
            TransactionRow(
                transaction: Transaction(
                    amount: 45.50, type: .expense,
                    category: Category.predefined[0], date: Date(), note: "Lunch at cafe"
                ),
                onEdit: { print("Edit tapped") }, onDelete: { print("Delete tapped") }
            )
            TransactionRow(
                transaction: Transaction(
                    amount: 1200.00, type: .income,
                    category: Category.predefined[1], date: Date(), note: nil
                ),
                onEdit: { print("Edit tapped") }, onDelete: { print("Delete tapped") }
            )
        }
