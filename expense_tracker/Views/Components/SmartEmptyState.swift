...
struct SmartEmptyState: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ZStack {
                Circle()
                    .fill(
                        LinearGradient(
                            colors: [type.accentColor.opacity(0.2), type.accentColor.opacity(0.05)],
        ...
        .padding()
        .onAppear {
            withAnimation(.spring(response: 0.6, dampingFraction: 0.7)) { animateIcon = true }
            withAnimation(.easeOut(duration: 0.5).delay(0.2)) { animateText = true }
        }
    }
}
...
enum EmptyStateType {
    ...
    var message: String {
        switch self {
        case .noTransactions:         return "Start tracking your finances by adding your first transaction"
        case .noFilteredTransactions: return "Try adjusting your filters or add new transactions"
        case .noStats:                return "Add some transactions to see your spending insights"
        case .noCategories:           return "Create categories to organize your transactions"
        }
    }
